...
Latest release is `v2.20.7`: the jar file (which includes the `math.geom2d` and `JavaHelp` jar files) - `drawfbp-2.20.7.jar` - can be obtained from the Releases folder, or from `build/libs`.  The previous release, `v2.20.6`, was published to Maven, but there is a bug that occurs when saving an image.  This has been fixed in v2.20.7. <!-- If you click on the Maven shield below to obtain DrawFBP, select `download` and `jar`. -->
