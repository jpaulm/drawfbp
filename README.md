...
Latest release is `v2.20.6`: the jar file (which includes the `math.geom2d` and `JavaHelp` jar files) - `drawfbp-2.20.6.jar` - can be obtained from the Releases folder, from `build/libs`, or from Maven.  <!--The previous release, `v2.20.4`, has been published to Maven, but there is always a slight lag, so for a while Maven may still be downloading `2.20.3`, even though the shield says `2.20.4`.--> If you click on the Maven shield below to obtain DrawFBP, select `download` and `jar`. 
