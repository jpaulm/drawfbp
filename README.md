...
Latest release is v2.19.1: the jar file (which includes the `math.geom2d` and JavaHelp jar files) - `drawfbp-2.19.1.jar` - can be obtained from the Releases folder, or from `build/libs`.  This jar file will be available on Maven shortly - you will then be able to click on the Maven shield below, and select `download` and `jar`.
