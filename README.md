...
Latest release is v2.18.6: the jar file (which includes the `math.geom2d` jar file) - `drawfbp-2.18.6.jar` - can be obtained from the Releases folder, or from `build/libs`.  The previous version on Maven (click on the Maven shield below) had some problems in managing directories for the Compile and Run functions.
