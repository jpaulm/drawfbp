...
Latest release is v2.18.6: the jar file (which includes the `math.geom2d` jar file) - `drawfbp-2.18.6.jar` - can be obtained from the Releases folder, or from `build/libs`.  It is also available from Maven - click on the Maven shield below, and select `download` and `jar`.
