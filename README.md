...
Latest release is `v2.19.6` - in pre-Release status. <!--: the jar file (which includes the `math.geom2d` and `JavaHelp` jar files) - `drawfbp-2.19.5.jar` - can be obtained from the Releases folder, or from `build/libs`.-->  The previous release, `v2.19.5`, has been published to Maven, but there is always a slight lag, so for a while Maven may still be downloading v2.19.4.  If you click on the Maven shield below to obtain DrawFBP, select `download` and `jar`. 
