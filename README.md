...
DrawFBP has a help facility which uses the powerful JavaHelp facility.  The first time you click on Help/Launch Help, you will be prompted to locate the `DrawFBP-Help.jar` file.  This is the standard JavaHelp jar file, and can be obtained from the `src\main\resources` folder in the DrawFBP jar file, or from http://www.jpaulmorrison.com/graphicsstuff/DrawFBP-Help.jar .
