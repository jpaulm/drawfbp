...
Latest release is `v2.20.9`: the jar file (which includes the `math.geom2d` and `JavaHelp` jar files) - `drawfbp-2.20.9.jar`.  This can be obtained from the Releases folder (click on `tags`, then `Releases`), or from `build/libs`.  <!-- If you click on the Maven shield below to obtain DrawFBP, select `download` and `jar`. -->
