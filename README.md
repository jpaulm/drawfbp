...
Latest release is v2.19.0: the jar file (which includes the `math.geom2d` and JavaHelp jar files) - `drawfbp-2.19.0.jar` - can be obtained from the Releases folder, or from `build/libs`.  This jar file is also available on Maven - click on the Maven shield below, and select `download` and `jar`.

Pre-release version v2.19.1 has fixed a couple of subtle bugs - yes, we are still finding them!
