...
An XML Schema has been added to the `lib` folder, called `drawfbp_files.xs`, specifying the format of the `.drw` files used to hold DrawFBP diagrams.  This schema can be used to check whether the file format of any other diagramming tool matches the `.drw` format.
